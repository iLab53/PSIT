...
Pharmaceutical Strategic Intelligence Tool -- ADC Oncology Pipeline Dashboard.
...
with tab1:
    ...
    st.subheader('Recently Updated Trials (last 90 days)')
...
# TAB 3: Regulatory
with tab3:
    with st.expander('Regulatory Signals -- FDA / EMA (Tier 2 Overlay)', expanded=True):
        st.caption(
        ...
        with get_connection() as conn:
            reg_df = pd.read_sql_query(
                ...
                'FROM regulatory_signals ORDER BY id DESC LIMIT 50',
                conn
            )
        if reg_df.empty:
    ...
    with st.expander('News Signals -- Endpoints / STAT (Tier 3 Overlay)', expanded=True):
        st.caption(
        ...
        with get_connection() as conn:
            news_df = pd.read_sql_query(
                'SELECT source_name, title, summary, published, source_url, signal_tier '
                'FROM news_signals ORDER BY id DESC LIMIT 50',
                conn
            )
        if news_df.empty:
...
with tab4:
    ...
    st.caption(
        'LLM-generated summary of validated structured data | '
        'Claude Haiku | All claims validated before display'
    )
    ...
    with get_connection() as conn:
        evidence_df = pd.read_sql_query(
            ...
            'FROM evidence_claims ORDER BY id DESC LIMIT 100',
            conn,
        )
